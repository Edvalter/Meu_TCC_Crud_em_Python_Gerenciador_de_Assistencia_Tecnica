...
class Funcao():
    ...
    def orcamento_Variaveis(self):
        self.id_orcamento = self.e_id_orcamento.get()
        self.id_cliente = self.e_id_cliente.get()
    ...
    def adiciona_Orcamento(self):
        self.orcamento_Variaveis()
        self.conecta_bd()
        self.cursor.execute("""                  
            INSERT INTO
                orcamentoteste (id_cliente, cpf, nome, whatsapp, produto, modelo, cor, defeito, observacao, data_entrada, data_retirada ) 
            VALUES 
                    ( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);""",
                    (self.id_cliente, self.cpf, self.nome, self.whatsapp, self.produto, self.modelo, self.cor, self.defeito, self.observacao, self.data_entrada, self.data_retirada))

        messagebox.showinfo(title="Orçamento", message="Orçamento Adicionado")
...
class Aplicacao_Orcamento(Funcao):
    ...
    def frames_Orcamento(self):
        ...
        self.frame_grid.place(x=10, y=250, height=230, width=1075)
    ...
    def labels_entry(self):
        # - - Frame Superior - -
        self.l_id_orcamento = Label(self.frame_superior, text="Orçamento:", font=("Courier", 13, "italic", "bold"), bg=co4, fg=co10)
        ...
        self.l_id_cliente = Label(self.frame_superior, text="Código Cliente:", font=("Courier", 13, "italic", "bold"), bg=co4, fg=co10)
        ...
        self.l_observacao = Label(self.frame_superior, text="Obersvações:", font=("Courier", 13, "italic", "bold"), bg=co4, fg=co10)
        ...
        self.l_data_entrada = Label(self.frame_superior, text="Entrada:", font=("Courier", 13, "italic", "bold"), bg=co4, fg=co10)
        self.l_data_entrada.place(x=540, y=60)
        self.e_data_entrada = DateEntry(self.frame_superior, width=42, justify='left', relief='solid', locale="pt_br")
        self.e_data_entrada.place(x=625, y=60)

        self.l_data_retirada = Label(self.frame_superior, text="Retirada:", font=("Courier", 13, "italic", "bold"), bg=co4, fg=co10)
        self.l_data_retirada.place(x=530, y=85)
        self.e_data_retirada = DateEntry(self.frame_superior, width=42, justify='left', relief='solid', locale="pt_br")
        self.e_data_retirada.place(x=625, y=85)
...
# - - Frame Inferior - -
    ...
    def botoes(self):
        ...
        self.b_procurarCliente = Button(self.frame_superior, text="Procurar", width=10, font=('Ivy 6 bold'), bg=co6, fg=co2, relief=RAISED, overrelief=RIDGE)
        self.b_procurarCliente.place(x=390, y=35, height=20, width=60)

        self.b_procurarProduto = Button(self.frame_superior, text="Procurar", width=10, font=('Ivy 6 bold'), bg=co6, fg=co2, relief=RAISED, overrelief=RIDGE)
        self.b_procurarProduto.place(x=390, y=110, height=20, width=60)

        self.b_adicionar = Button(self.frame_superior, text="Adicionar", command=self.adiciona_Orcamento, width=10, font=('Ivy 8 bold'), bg=co7, fg=co2, relief=RAISED, overrelief=RIDGE)
